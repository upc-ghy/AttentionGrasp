...
class GraspDataset(Dataset):
    ...
    def get_data(self, index, return_raw_cloud=False):
        ...
        # 点云中每个点的坐标
        # ret_dict['coord'] = cloud_sampled.astype(np.float32)
        # 依据GridSampling得到的每个点的网格坐标
        # ret_dict['grid_size'] = 0.005
        # feat表示每个点的颜色值+该点法向量(color+normal)
        ret_dict['feat'] = np.concatenate([cloud_sampled.astype(np.float32), color_sampled.astype(np.float32), normal_sampled.astype(np.float32)], axis=1)
        return ret_dict
