...
from dataset_processed import GraspDataset, collate_fn
from loss_utils import generate_grasp_views, batch_viewpoint_params_to_matrix

parser = argparse.ArgumentParser()
parser.add_argument('--dataset_root', required=True, help='Dataset root')
parser.add_argument('--camera', required=True, help='Camera split [realsense]')
...
parser.add_argument('--optim_log', required=True,)
...
class CmapEval(GraspNetEval):
    ...
    def opt(self):
        ...
        for batch_idx, batch_data in enumerate(TEST_DATALOADER):
            ...
            gg_file = os.path.join(cfgs.test_log, SCENE_LIST[batch_idx], cfgs.camera, "result.npy")
            res_file = os.path.join(cfgs.test_log, SCENE_LIST[batch_idx], cfgs.camera,"save.npy")
            ...
            save_dir = os.path.join(cfgs.optim_log, SCENE_LIST[batch_idx], cfgs.camera)
            grasp_refined_list = []
